...
export class Versions extends Instantiable {
  ...
  public async get(): Promise<PlatformVersions> {
    ...
    versions.sdk = {
      ...
      network: await this.nevermined.keeper.getNetworkName(),
      keeperVersion: this.nevermined.keeper.getNetworkVersion(),
    ...
    // Metadata
    try {
      const { software: name, version } = await this.nevermined.services.metadata.getVersionInfo()
      versions.metadata = {
        name,
        status: PlatformTechStatus.Working,
        version,
