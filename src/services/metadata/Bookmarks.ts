import { MarketplaceApi, buildQuery, HttpError, ApiError } from '@/sdk'
...
export class Bookmarks extends MarketplaceApi {
   ...
   */
  public async updateOneById(id: string, bookmark: Partial<Bookmark>): Promise<Bookmark> {
    const fullUrl = `${this.url}${bookmarkPath}/${id}`

    try {
      const response = await this.nevermined.utils.fetch.put(fullUrl, JSON.stringify(bookmark), {
        Authorization: `Bearer ${this.config.marketplaceAuthToken}`,
      })
