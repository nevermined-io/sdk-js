...
describe('DIDSalesTemplate', () => {
  ...
  describe('create agreement', () => {
    ...
    it('should fail if DID is not registered', async () => {
      ...
      await templateStoreManager.proposeTemplate(didSalesTemplate.address, deployer)
      await templateStoreManager.approveTemplate(didSalesTemplate.address, deployer)
      const did = await didRegistry.hashDID(didSeed, sender.getId())
    ...
    it('should create agreement', async () => {
      ...
      const txReceipt = await didSalesTemplate.createAgreement(
        agreementIdSeed,
        didZeroX(did),
        conditionIdSeeds,
        timeLocks,
      ...
      const logs = didSalesTemplate.getTransactionLogs(txReceipt, 'AgreementCreated')
