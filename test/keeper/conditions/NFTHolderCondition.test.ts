...
describe('NFTHolderCondition', () => {
    ...
    describe('#hashValues()', () => {
        it('should hash the values', async () => {
            const hash = await nftHolderCondition.hashValues(
                did, holder.getId(), amount)
    ...
    describe('fulfill existing condition', () => {
        it('should fulfill if conditions exist for account address', async () => {
            ...
            await didRegistry.registerMintableDID(
                did, checksum, [], value, activityId, '', 100, 0, owner.getId())
            await didRegistry.mint(did, 10, owner.getId())
            await didRegistry.transferNft(did, holder.getId(), 10, owner.getId())

            const result = await nftHolderCondition.fulfill(agreementId, did, holder.getId(), amount)
            const { state } = await conditionStoreManager.getCondition(conditionId)
            assert.equal(state, ConditionState.Fulfilled)

            const { _agreementId, _did, _address, _conditionId, _amount } = result.events.Fulfilled.returnValues
    ...
    describe('fulfill non existing condition', () => {
        it('should not fulfill if conditions do not exist', async () => {
            ...
            await assert.isRejected(nftHolderCondition.fulfill(agreementId, did, holder.getId(), amount),
                /Invalid UpdateRole/
            )
        })
    })
