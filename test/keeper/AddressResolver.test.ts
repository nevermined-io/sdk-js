...
import {
    ...
    getContractAddress } from "../../src/keeper/AddressResolver"
...
describe('AddressResolver', () => {
    describe('#createAddressBookFromArtifacts()', () => {
        it('expect empty mapping when no artifacts are found in the given folder', () => {
            assert.equal(createAddressBookFromArtifacts('../testdata'), {})
        })
        it('extract the network-contract-address mapping correctly', () => {
            assert.equal(
                createAddressBookFromArtifacts('../testdata/artifacts'),
                {
                    mainnet: {
    ...
    describe('#getAddressBook()', () => {
        it('get the default addressBook', () => {
            ...
            assert.equal(addressMap.mainnet.AccessCondition, "0xBa635a16ad65fc44776F4577E006e54B739170e1")
            assert.equal(addressMap.mainnet.AccessTemplate, "0x5cc43778946671Ab88Be0d98B2Bc25C0c67095bb")
        })
        it('get the addressBook using file from config', () => {
            assert.equal(
                getAddressBook(config),
                {
                    "rinkeby": {
    ...
    describe('#getContractAddress()', () => {
        it('get contract address at specific network', () => {
            assert.equal(getContractAddress(config, "AccessCondition", "rinkeby"), "0x6fD85bdc2181955d1e13e69cF6b7D823065C3Ca7")
        })
