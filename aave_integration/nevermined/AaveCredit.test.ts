...
describe('AaveCredit', () => {
  ...
  before(async () => {
    ...
    if (nftAddress.toString() !== '') {
      ...
      nftContract = await TestContractHandler.deployArtifact(ERC721, deployer.getId(), [
        deployer.getId(),
        didRegistry.address,
        'NFT721',
        'NVM',
      ...
      nft721Wrapper = (await nevermined.contracts.loadNft721(nftContract.address)).getContract
    }
  ...
  describe('Create a credit NFT collateral agreement', function () {
    ...
    it('Create a credit agreement', async () => {
      const didRegAddress = await agreementStoreManager.call('getDIDRegistryAddress', [])
      ...
      if (didBlockNumber === null || didBlockNumber === undefined || didBlockNumber === 0) {
    ...
    it('The borrower locks the NFT', async () => {
      ...
      if (_condition.state !== ConditionState.Fulfilled) {
        ...
        await nevermined.services.aave.lockNft(agreementId, nftContractAddress, nftAmount, borrower)
    ...
    it('A second NFT cant be locked into the Vault', async () => {
      ...
      await nft721Wrapper.send('approve', borrower.getId(), [vaultAddress, zeroX(_did)])
    ...
    it('Lender deposits ETH as collateral in Aave and approves borrower to borrow DAI', async () => {
      const { state: _stateDeposit } = await conditionStoreManager.getCondition(conditionIds[1])
      ...
      if (_stateDeposit !== ConditionState.Fulfilled) {
        ...
        const { state: stateDeposit } = await conditionStoreManager.getCondition(conditionIds[1])
    ...
    it('Borrower/Delegatee borrows DAI from Aave on behalf of Delegator', async () => {
      const { state: _stateBorrow } = await conditionStoreManager.getCondition(conditionIds[2])
      if (_stateBorrow !== ConditionState.Fulfilled) {
        ...
        assert.isTrue(success)
        const { state: stateCredit } = await conditionStoreManager.getCondition(conditionIds[2])
    ...
    it('Borrower/Delegatee repays the loan with DAI', async () => {
      const { state: _stateRepay } = await conditionStoreManager.getCondition(conditionIds[3])
      if (_stateRepay !== ConditionState.Fulfilled) {
        const totalDebt = await nevermined.services.aave.getTotalActualDebt(agreementId, borrower)
        ...
        await dai.send('transfer', daiProvider, [borrower.getId(), transferAmount])
        ...
        const vaultBalancesAfter = await nevermined.services.aave.getActualCreditDebt(
          agreementId,
          borrower,
        )
    ...
    it('Delegator withdraw collateral and fees', async () => {
      const { state: _stateWithdraw } = await conditionStoreManager.getCondition(conditionIds[4])
      if (_stateWithdraw !== ConditionState.Fulfilled) {
        ...
        assert.isTrue(ethBalanceAfter.sub(ethBalanceBefore).sub(collateralAmount).gt(0))
      }
    })

    it('Borrower/Delegatee paid the credit so will get back the NFT', async () => {
      assert.isTrue(
        await nevermined.services.aave.unlockNft(agreementId, nftContractAddress, borrower),
      )
