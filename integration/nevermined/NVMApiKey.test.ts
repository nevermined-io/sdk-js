...
describe('Nevermined API Key', () => {
  ...
  describe('As a provider I can generate and process Nevermined API Keys', () => {
    ...
    before(async () => {
      ...
      ;[user, provider, someone] = nvm.accounts.list()
    ...
    it.skip('The token can be encoded & decoded', async () => {
      ...
      const apiKey = NvmApiKey.deserialize(serialized)
      console.log(apiKey)
    ...
    it('Encrypt and decrypt', async () => {
      ...
      const apiKey = NvmApiKey.deserialize(decrypted)
      console.log(apiKey)
      assert.equal(apiKey.iss, user.getId())
      assert.equal(apiKey.sub, providerAddress)
    })
    ...
    it.skip('The api token can no be decripted by a different account to the receiver', async () => {
      ...
      assert.isDefined(encryptedNvmApiKey)
      try {
        await NvmApiKey.decryptAndDecode(encryptedNvmApiKey, someonePrivateKey)
    ...
    it.skip('LOCAL: As a user I can generate a NVM API Key for the NODE', async () => {
      ...
      const apiKey = await NvmApiKey.decryptAndDecode(encryptedNvmApiKey, privateKey)
      assert.isDefined(apiKey)
    ...
    it('I can generate 2 session keys and they are different', async () => {
      ...
      const hash1 = NvmApiKey.hash(key1)
      const hash2 = NvmApiKey.hash(key2)
