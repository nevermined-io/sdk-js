...
describe('Compute Asset', () => {
  ...
  it('should execute the compute service', async () => {
    workflowId = await nevermined.compute.execute(agreementId, workflowDdo.id, consumer)
    assert.isDefined(workflowId)
    const result = await getResultDidFromStatus(workflowId, agreementId, consumer, nevermined)
