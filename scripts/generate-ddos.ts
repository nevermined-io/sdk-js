...
import { RoyaltyKind } from '../src/nevermined/api/RoyaltyKind'
...
const generateDatasetDDO = async (
    ...
    fs.writeFileSync(
        `${destination}/metadata-dataset.json`,
...
const generateAlgorithmDDO = async (
    ...
    delete metadata.userId
    fs.writeFileSync(
        `${destination}/metadata-algorithm.json`,
        JSON.stringify(metadata, undefined, 2)
    )
...
const generateWorkflowDDO = async (
                ...
                stages: [
                    {
                        ...
                        output: {
                            ...
                            accessProxyUrl:
                                'https://node.mumbai.public.nevermined.network/api/v1/gateway/',
                            metadata: {} as any
                        }
                    }
                ]
    ...
    delete metadata.userId
    fs.writeFileSync(
        `${destination}/metadata-workflow.json`,
        JSON.stringify(metadata, undefined, 2)
    )
...
const generateComputeDDO = async (
    ...
    delete metadata.userId
    fs.writeFileSync(
        `${destination}/metadata-compute.json`,
        JSON.stringify(metadata, undefined, 2)
    )
